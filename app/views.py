    ...
    try:
        ...
        if category == 'Rivanna HPC':
            ...
            if (request_type in ['new-storage', 'increase-storage', 'decrease-storage']):
                ...
                elif storage_choice == 'High-Security Research Project':
                    table_name = app.config['HIGH_SECURITY_PROJECT_STORAGE_REQUEST_INFO_TABLE']
                    app.logger.info(f"table_name:{table_name}")
                    update_high_security_project_storage_request_info_table(ticket_response, form_elements_dict, desc_str, project_ticket_route, table_name)
                else:
...
def update_high_security_project_storage_request_info_table(ticket_response, form_elements_dict, desc_str, project_ticket_route, table_name):
    try:
        ticket_id = json.loads(ticket_response)['issueKey']
        today = datetime.datetime.now()
        formatted_date = today.strftime("%Y-%m-%d %H:%M:%S")
